...
async def create_session(
    session_data: SessionCreate, request: Request, db: AsyncSession = Depends(get_db)
):
    ...
    if session_data.latitude is not None and session_data.longitude is not None:
        try:
            location_hash = create_location_hash(
    ...
    session, host_token = await service.create_session(
        session_data.host_name, network_hash=network_hash, location_hash=location_hash
    )
...
async def upload_receipt(
    ...
    logger.info(
        f"Processing receipt upload for session {code}", extra={"file_size": total_size}
    )
    ...
    try:
        ...
        for item_data in result["items"]:
            ...
            if (
                quantity > 1
                and not item_data["is_tax"]
                and not item_data["is_tip_suggestion"]
                and not is_refund
            ):
        ...
        await db.commit()
        raise HTTPException(
            status_code=500, detail=f"Failed to process receipt: {str(e)}"
        )
...
async def get_session_summary(code: str, db: AsyncSession = Depends(get_db)):
    ...
    result = calculator.calculate_full_split(
        items, participants, assignments, discounts
    )
    ...
    tax_total = sum(item.price * item.quantity for item in session.items if item.is_tax)
