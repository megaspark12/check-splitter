...
class BillCalculator:
    ...
    def _distribute_with_remainder(
        self, total: Decimal, shares: dict[str, int]
    ) -> dict[str, Decimal]:
        """
        ...
        for i, (participant_id, share_count) in enumerate(sorted_participants):
            if i == len(sorted_participants) - 1:
                ...
                share = self._round(
                    total * Decimal(share_count) / Decimal(total_shares)
                )
    ...
    def calculate_item_share(
        self, item: ItemProtocol, assignments: list[AssignmentProtocol]
    ) -> dict[str, Decimal]:
        """
    ...
    def distribute_tax(
        self, participant_subtotals: dict[str, Decimal], tax_total: Decimal
    ) -> dict[str, Decimal]:
        """
        ...
        sorted_participants = sorted(
            participant_subtotals.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def calculate_full_split(
        ...
        regular_items = [
            i
            for i in items
            if not i.is_tax and not i.is_tip_suggestion and not i.is_refund
        ]
        ...
        participant_subtotals = {
            pid: data["items_subtotal"] for pid, data in participant_data.items()
        }
        ...
        for discount in participant_discounts:
            if discount.participant_id in participant_data:
                ...
                discount_amount = self.calculate_discount(
                    data["items_subtotal"], discount.discount_type, discount.value
                )
        ...
        total_bill_subtotal = sum(
            d["items_subtotal"] for d in participant_data.values()
        )
            ...
            if discount.discount_type == "percentage":
                ...
                if total_bill_subtotal > Decimal("0"):
                    total_discount = self.calculate_discount(
                        total_bill_subtotal, discount.discount_type, discount.value
                    )
