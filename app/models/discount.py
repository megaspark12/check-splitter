...
class Discount(Base):
    ...
    session_id = Column(CHAR(36), ForeignKey("sessions.id", ondelete="CASCADE"), nullable=False)
    
    # Optional: if set, discount applies only to this participant
    # If null, discount applies to entire bill (split proportionally)
    participant_id = Column(CHAR(36), ForeignKey("participants.id", ondelete="CASCADE"), nullable=True)
    ...
    def __repr__(self):
        target = f"participant={self.participant_id}" if self.participant_id else "entire bill"
