...
def test_full_flow():
    ...
    r = requests.post(
        f"{BASE}/sessions/{code}/items",
        json={"name": "Burger", "price": 12.99, "quantity": 1},
    )
    assert r.status_code == 201
    burger = r.json()
    r = requests.post(
        f"{BASE}/sessions/{code}/items",
        json={"name": "Pizza", "price": 15.50, "quantity": 1},
    )
    assert r.status_code == 201
    pizza = r.json()
    r = requests.post(
        f"{BASE}/sessions/{code}/items",
        json={"name": "Tax", "price": 2.50, "quantity": 1, "is_tax": True},
    )
    assert r.status_code == 201
    ...
    r = requests.post(
        f"{BASE}/sessions/{code}/assignments",
        json={"item_id": burger["id"], "participant_id": host_id, "share_count": 1},
    )
    assert r.status_code == 201
    r = requests.post(
        f"{BASE}/sessions/{code}/assignments",
        json={"item_id": pizza["id"], "participant_id": bob["id"], "share_count": 1},
    )
    assert r.status_code == 201
    ...
    # 5. Update Bob tip
    r = requests.put(
        f'{BASE}/sessions/{code}/participants/{bob["id"]}', json={"tip_percentage": 18}
    )
    ...
    # 6. Add discount
    r = requests.post(
        f"{BASE}/sessions/{code}/discounts",
    ...
    for p in summary["participants"]:
        print(
            f'   {p["participant_name"]}: items={p["items_subtotal"]}, tax={p["tax_share"]}, tip={p["tip_amount"]}, discount={p["discount_amount"]}, total={p["total"]}'
        )
    ...
    print(
        f'✅ Session data complete: {len(session["items"])} items, {len(session["participants"])} participants, {len(session["discounts"])} discounts'
    )

    # 9. Test duplicate assignment (race condition guard)
    r = requests.post(
        f"{BASE}/sessions/{code}/assignments",
        json={"item_id": burger["id"], "participant_id": host_id, "share_count": 2},
    )
    assert r.status_code == 201
