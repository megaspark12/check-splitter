...
def _create_session(page, base_url):
    ...
    expect(page.locator("#session-page")).to_have_class(
        re.compile("active"), timeout=TIMEOUT
    )
...
def _host_with_items(page, base_url, api_url):
    ...
    expect(page.locator("#session-page")).to_have_class(
        re.compile("active"), timeout=15000
    )
...
class TestHostWithItems:
    ...
    def test_host_selects_item(self, page: Page, base_url, api_url):
        _host_with_items(page, base_url, api_url)
        page.locator(
            "#items-list .item-row:not(.tax-item) .item-checkbox"
        ).first.click()
        expect(page.locator("#items-list .item-row.selected")).to_have_count(
            1, timeout=TIMEOUT
        )
...
class TestHostQRCode:
    ...
    def test_qr_modal_opens(self, page: Page, base_url):
        ...
        expect(page.locator("#qr-modal")).to_have_class(
            re.compile("active"), timeout=TIMEOUT
        )
    ...
    def test_qr_modal_closes(self, page: Page, base_url):
        ...
        expect(page.locator("#qr-modal")).not_to_have_class(
            re.compile("active"), timeout=TIMEOUT
        )
