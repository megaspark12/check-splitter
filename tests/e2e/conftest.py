...
def live_server(server_port, base_url):
    ...
    project_root = os.path.join(os.path.dirname(__file__), '..', '..')
    for f in ('test_e2e.db', 'test_e2e.db-wal', 'test_e2e.db-shm',
              'test_server.log', 'test_server_err.log'):
...
def add_items_via_api(api_url, create_session_via_api):
    ...
    def _add(code, items, host_token=None):
        ...
        results = []
        for item in items:
            r = requests.post(f"{api_url}/sessions/{code}/items", json=item, headers=headers)
            assert r.status_code == 201
