...
class TestParticipantJoins:
    ...
    def test_participant_sees_items(
        self, page: Page, base_url, create_session_via_api, add_items_via_api
    ):
        session = create_session_via_api("Alice")
        code = session["code"]
    ...
    def test_participant_name_displayed(
...
class TestParticipantSelectsItems:
    ...
    def test_total_updates(
        ...
        add_items_via_api(
            code,
            [{"name": "Burger", "price": 10.00}, {"name": "Pizza", "price": 20.00}],
            host_token=host_token,
        )
        _join(page, base_url, code)
        expect(page.locator("#participant-items-list .item-row")).to_have_count(
    ...
    def test_tip_buttons(
        ...
        host_token = session["host_token"]
        add_items_via_api(
            code, [{"name": "Burger", "price": 10.00}], host_token=host_token
        )
        _join(page, base_url, code)
...
class TestParticipantLeave:
    ...
    def test_leave_returns_to_landing(
        self, page: Page, base_url, create_session_via_api
    ):
        session = create_session_via_api("Alice")
        _join(page, base_url, session["code"])
        page.locator("#participant-leave-session-btn").click()
        expect(page.locator("#confirm-modal")).to_have_class(
            re.compile("active"), timeout=3000
        )
        page.locator("#confirm-ok").click()
        expect(page.locator("#landing-page")).to_have_class(
            re.compile("active"), timeout=5000
        )
