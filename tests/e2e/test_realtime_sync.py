...
class TestRealTimeSync:
    ...
    def test_host_sees_participant_join(self, browser: Browser, base_url):
        ...
        expect(host_page.locator("#session-page")).to_have_class(
        ...
        expect(part_page.locator("#participant-page")).to_have_class(
            re.compile("active"), timeout=5000
        )

        expect(host_page.locator("#participants-count")).to_have_text(
            "2", timeout=10000
        )
        host_ctx.close()
        part_ctx.close()
    ...
    def test_participant_sees_new_item(self, browser: Browser, base_url, api_url):
        ...
        expect(host_page.locator("#items-list .item-row:not(.tax-item)")).to_have_count(
            1, timeout=5000
        )
        ...
        expect(part_page.locator("#participant-items-list .item-row")).to_have_count(
            1, timeout=5000
        )
    ...
    def test_multiple_participants(self, browser: Browser, base_url, api_url):
        ...
        expect(p1.locator("#participant-page")).to_have_class(
            re.compile("active"), timeout=5000
        )
        ...
        expect(p2.locator("#participant-page")).to_have_class(
            re.compile("active"), timeout=5000
        )

        expect(p1.locator("#participant-items-list .item-row")).to_have_count(
            2, timeout=5000
        )
        expect(p2.locator("#participant-items-list .item-row")).to_have_count(
            2, timeout=5000
        )
        expect(host_page.locator("#participants-count")).to_have_text(
            "3", timeout=10000
