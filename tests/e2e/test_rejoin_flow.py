...
class TestRejoinBanner:
    ...
    def test_auto_rejoin_host_after_refresh(self, page: Page, base_url):
        ...
        expect(page.locator("#session-page")).to_have_class(re.compile("active"), timeout=10000)

    def test_auto_rejoin_participant_after_refresh(self, page: Page, base_url, create_session_via_api):
        ...
        page.reload()
        expect(page.locator("#participant-page")).to_have_class(re.compile("active"), timeout=10000)
    ...
    def test_participant_name_preserved_after_rejoin(self, page: Page, base_url, create_session_via_api):
...
class TestDataPersistence:
    ...
    def test_items_persist_host(self, page: Page, base_url, api_url):
        ...
        page.reload()
        expect(page.locator("#session-page")).to_have_class(re.compile("active"), timeout=10000)
        expect(page.locator("#items-list .item-row:not(.tax-item)")).to_have_count(1, timeout=5000)

    def test_selections_persist_participant(self, page: Page, base_url, create_session_via_api, add_items_via_api):
        session = create_session_via_api("Alice")
        code = session["code"]
        host_token = session["host_token"]
        add_items_via_api(code, [{"name": "Burger", "price": 12.99}, {"name": "Pizza", "price": 15.50}], host_token=host_token)
        page.goto(base_url)
