...
class TestHostTokenAuthentication:
    ...
    async def test_remove_participant_requires_host_token(self, client: AsyncClient):
        ...
        join_response = await client.post(
            f"/api/sessions/{code}/participants",
            json={"name": "Bob"}
        )
        participant_id = join_response.json()["id"]
        
        # Try to remove without token
        response = await client.delete(f"/api/sessions/{code}/participants/{participant_id}")
        assert response.status_code == 401
    ...
    async def test_read_operations_do_not_require_host_token(self, client: AsyncClient):
        ...
        assert (await client.get(f"/api/sessions/{code}/participants")).status_code == 200
...
class TestSessionExpiry:
    ...
    @pytest.mark.asyncio
    async def test_expired_session_returns_410(self, client: AsyncClient, db_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_expired_session_blocks_all_operations(self, client: AsyncClient, db_session: AsyncSession):
        ...
        assert (await client.get(f"/api/sessions/{code}/participants")).status_code == 410
